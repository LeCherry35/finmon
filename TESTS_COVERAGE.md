...
| ✅ DB queries (mocked) | `src/db/queries.test.ts` | Generated SQL + param array for each branch (category/month clauses, day-vs-month bucket, empty-month short-circuit), the `getTransactions` products-attach round-trip, receipt LEFT JOIN and `date`/`added` sort ordering, the uncategorized-spend handling (expenditure queries LEFT JOIN + coalesce the null bucket to "Uncategorized"; the plan/summary queries append a synthetic id-0 row only when there's uncategorized spend and no category filter), `getReceiptImage` (user-scoped, null for missing/foreign), and a sweep asserting every query is scoped by `user_id = $1` |
