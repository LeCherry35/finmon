...
// Product write logic, keyed by an explicit `userId`. The "use server" actions
// in src/actions/products.ts delegate here, as do the receipt-scan action and
// the agent's proposal-apply path. These take a userId, so they must never be
// exported from a "use server" module (a client could pass any user's id).
 ...
 *  add/update/delete; also used by the receipt-scan action. */
...
export async function addProductFor(
  userId: string,
  formData: FormData,
...
export async function updateProductFor(
  userId: string,
  formData: FormData,
): Promise<ActionResult> {
...
export async function deleteProductFor(
  userId: string,
  formData: FormData,
): Promise<ActionResult> {
  const id = Number(formData.get("id"));
