...
- "total" is the receipt grand total — the final amount paid, with every discount (per-item or check-wide) already reflected — as a positive number with no currency symbol, or null if you cannot read it. Never subtract anything from the printed total yourself, and never report a discount already included in it anywhere else.
...
- "products" is one object per purchased line item. Do NOT create a product for subtotals, totals, tax, discounts, loyalty messages, or store metadata — those are not products (the grand total belongs in "total"; a discount printed against a specific item belongs on that product's "discount"; a check-wide discount — loyalty card, coupon, order-level promotion — is not extracted at all, it is already reflected in "total").
