...
`scanReceiptForTransaction(formData)` in `src/actions/receipt.ts` (fields `transaction_id`, `image` = a `data:image/…` URL) attaches AI-parsed products to an existing transaction. It validates the id and `userOwnsTransaction`, calls `scanReceipt` (`src/lib/receipt-scan.ts` — one OpenAI vision call, json_schema structured output, zod-validated → `ProductFields[]`), bulk-inserts via `insertProducts`, and recomputes status. The recompute + `revalidatePath("/transactions")` run in a `finally`, so a failed scan returns `{ ok:false, error }` **and** still clears a `processing` row back to `unverified` (never stuck mid-scan). Used by the create flow (after `createTransaction` starts the row `processing` via the `has_receipt` flag and returns its new id as `lastTxId`) and by the products modal. The image is never persisted — the client downscales it and passes it straight to this action.
