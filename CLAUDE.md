...
Pages: `/transactions` (default landing — list/CRUD), `/categories` (CRUD), `/plan` (planned vs spent vs left, per category), `/expenditures` (spend totals by category; each category links to its filtered transactions), `/charts` (visualizations — a stacked-area expenditures-over-time chart and a category-share donut, switchable via tabs as more chart types are added).
