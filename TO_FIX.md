...
### `createTransaction` `ROLLBACK` in `catch` can mask the original error
`src/actions/transactions.ts` `createTransaction` does `catch (e) { await client.query("ROLLBACK"); throw e }`. If the `ROLLBACK` itself throws (e.g. dead connection), that error propagates and the original cause `e` is lost. Capture/log `e` before attempting the rollback, or wrap the rollback in its own try/catch so the root cause always surfaces.
...
### `createTransaction` category upsert runs outside the client transaction
`src/actions/transactions.ts` `createTransaction` upserts the category via a bare `pool.query` *before* opening the `BEGIN`/`COMMIT` block for the transaction + default product. If that block rolls back, the category persists (orphan category with no transaction — harmless but inconsistent), and it costs an extra pool round-trip. Folding the upsert into the same client transaction would make all three writes atomic and save a round-trip.
